...
def run():
    ...
    # config
    config = yaml.load(open("./config/envs_agw.yml", 'r'), Loader=yaml.SafeLoader)
    ...
    # plot
    utils.plot_bnn_regression_uncertainty(result, tau_out=config["TAU_OUT_VAL"], \
        file_name=os.path.join(result_dir, "bnn_regression_" + datetime.today().strftime("%Y%m%d%H%M")) + ".png")
    ...
    utils.plot_bnn_learning_curves(
        ...
        pred_list_tr, \
        result['y_train'], tau_out=config["TAU_OUT_VAL"], \
